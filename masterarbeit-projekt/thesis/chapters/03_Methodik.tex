...
Für Experiment~2 wird gezielt der Zusammenhang zwischen der Variable \texttt{OverTime} und der Zielvariable \texttt{Attrition} verändert. Die Wahl von \texttt{OverTime} erfolgt, da die Variable in Experiment~1 von beiden Modellen wiederholt als relevanter Faktor für Attrition aufgegriffen wurde und aufgrund ihrer binären Ausprägung eine eindeutige Veränderung der Richtung ermöglicht. Im Originaldatensatz beträgt die Attritionsquote bei Beschäftigten mit \texttt{OverTime = Yes} 30,5~\%, gegenüber 10,4~\% bei \texttt{OverTime = No}. Im manipulierten Datensatz wird die Richtung dieses Zusammenhangs umgekehrt, sodass Beschäftigte ohne Überstunden die höhere Attritionsquote aufweisen.

Die Manipulation erfolgt durch eine Neuzuordnung der Werte der Spalte \texttt{OverTime}. Dabei bleiben sowohl die Randverteilung von \texttt{OverTime} als auch die Zielvariable \texttt{Attrition} und sämtliche übrigen Variablen gegenüber dem Originaldatensatz unverändert. Verändert wird ausschließlich die gemeinsame Verteilung von \texttt{OverTime} und \texttt{Attrition}. Dieses Vorgehen wird einer Veränderung der Kündigungslabels vorgezogen, da eine solche auch die Zusammenhänge zwischen \texttt{Attrition} und weiteren Variablen beeinflussen würde. Durch die Neuzuordnung von \texttt{OverTime} bleiben demgegenüber die Referenzwerte derjenigen Fragen unverändert, deren Beantwortung nicht von dieser Variable abhängt. Die Manipulation ist damit gezielt auf den zu untersuchenden Zusammenhang beschränkt.

Für die Auswertung wird zwischen Analyse- und Kontrollfragen unterschieden. Frage~A09 bildet die direkte Prüfgröße, da sie explizit nach der Richtung des Zusammenhangs zwischen \texttt{OverTime} und \texttt{Attrition} fragt. Ergänzend werden die interpretativen Fragen~B01, B02, B03 und B05 betrachtet, in deren Antworten \texttt{OverTime} bereits in Experiment~1 regelmäßig als relevanter Faktor aufgegriffen wurde. Frage~B04 wird ergänzend betrachtet, da \texttt{OverTime} dort in Experiment~1 seltener thematisiert wurde. Die übrigen Fragen dienen als Kontrollfragen, da ihre für die Beantwortung relevanten Datengrundlagen durch die Manipulation unverändert bleiben.

Für die Analysefragen wird kein Punktwert vergeben. Stattdessen wird jede Antwort ihrer Aussage zum Zusammenhang zwischen \texttt{OverTime} und \texttt{Attrition} einer von drei Kategorien zugeordnet. Als \emph{folgt den Daten} gilt eine Antwort, deren Aussage der Richtung im manipulierten Datensatz entspricht. Als \emph{folgt der ursprünglichen Richtung} wird eine Antwort klassifiziert, die weiterhin den in Experiment~1 beobachteten Zusammenhang wiedergibt und damit der manipulierten Datengrundlage widerspricht. Antworten, aus denen keine eindeutige Aussage zur Richtung des Zusammenhangs hervorgeht, werden als \emph{uneindeutig} klassifiziert. Bei den interpretativen Fragen der Kategorie~B wird zusätzlich erfasst, ob \texttt{OverTime} überhaupt als relevanter Faktor genannt wird.

Die Kategorien beziehen sich ausschließlich auf die beobachtbare Übereinstimmung der Antworten mit der jeweiligen Datengrundlage. Aus einem Festhalten an der ursprünglichen Richtung kann nicht unmittelbar geschlossen werden, ob dieses Verhalten auf ein Auswendiglernen des konkreten Datensatzes, allgemeinem Domänenwissen oder anderen Einflüssen beruht. Die Bewertung beschränkt sich daher auf das beobachtbare Antwortverhalten; die Einordnung möglicher Ursachen erfolgt in der Diskussion. Für jedes Modell wird über die fünf Wiederholungen erfasst, wie häufig die Antworten den manipulierten Daten, der ursprünglichen Richtung oder keiner der beiden Richtungen eindeutig folgen.
